...
import { logout, getGroup, isLoggedIn } from '../utils/api'
...
export default function Layout() {
  ...
  // 加载用户 + 按需启动同步
  useEffect(() => { loadUser(); loadCustomCats(); if (isLoggedIn()) initSync(); return () => teardownSync() }, [])
    ...
    <div className="flex flex-col h-dvh bg-white dark:bg-gray-900 max-w-lg mx-auto">
      ...
      <div className="flex items-center justify-between px-4 pt-2 shrink-0">
        {isLoggedIn() ? (
          ...
          <span className="text-xs text-gray-400">本地模式</span>
        )}
        <div className="flex items-center gap-2">
          ...
          {isLoggedIn() ? (
            ...
            <button onClick={() => navigate('/login')} className="text-xs text-blue-500 hover:text-blue-700">登录同步</button>
          )}
        </div>
      </div>
      {/* Group info */}
