...
const KEYWORD_CATEGORY_MAP: Record<string, { catId: string; type: 'expense' | 'income' }> = {
  ...
  // 支付类默认
  '微信支付|付款|扫码支付|支付|收款|转账': { catId: 'other_exp', type: 'expense' },
}

// 金额模式
const AMOUNT_RE = /(\d+(?:\.\d{1,2})?)\s*[元块¥]\s*(\d+)?\s*(?:毛|角|钱)?|(\d+(?:\.\d{1,2})?)\s*(?:块钱|元钱)|([一二两三两四五六七八九十百千万亿零]+)\s*[块元]\s*([一二两三两四五六七八九])?\s*(?:毛|角|钱)?|(\d+(?:\.\d{1,2})?)(?=\s|$|[^\d])|(?:微信支付|付款|支付|收款|转账|红包)\s*[¥：:]*\s*(\d+(?:\.\d{1,2})?)/g
